...
function EditListModal (props) {
	...
	const removeCreator = (creator) => {
		Meteor.call('lists.removeCreator', props.list._id, creator )
	}

	// Delete List on button click
	const removeList = () => {
		Meteor.call('lists.remove', props.list._id, (err, res) => {
			if(!err) {
	...
	const onTitleBlur = () => {
		...
		Meteor.call('lists.updateTitle', props.list._id, title, (err, res) => {
			if(err) {
		...
		<BasicModal closeOnClickOutside={props.setIsModalVisible} isVisible={props.isModalVisible}>
			{props.list &&
				<EditListModalContainer>
					...
					{!props.noList &&
						<List>
							{props.list.creators.map((creator, index) => {
								return (
									<React.Fragment>
